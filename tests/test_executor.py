...
from gestnova_shell.executor import exec_task, tail_audit
...
def test_exec_ls_succeeds(sandbox):
    r = exec_task("read", "ls -la", str(sandbox))
    assert r.ok is True
    assert r.exitCode == 0
...
def test_exec_echo_returns_stdout(sandbox):
    r = exec_task("read", "echo hola", str(sandbox))
    assert r.ok is True
    assert "hola" in r.stdout


def test_exec_unauthorized_command_blocked(sandbox):
    r = exec_task("read", "rm -rf /tmp/foo", str(sandbox))
...
def test_exec_cwd_outside_allowed_blocked(sandbox):
    ...
    assert r.ok is False
    assert "allowed root" in r.reason.lower() or "not under" in r.reason.lower()


def test_exec_pipe_in_argv_blocked(sandbox):
    r = exec_task("read", "ls | rm", str(sandbox))
    assert r.ok is False


def test_exec_path_traversal_blocked(sandbox):
    r = exec_task("read", "cat ../../etc/passwd", str(sandbox))
    assert r.ok is False
...
def test_exec_timeout_enforced(sandbox):
    ...
    r = exec_task("read", "echo done", str(sandbox), timeout_s=5)
    assert r.ok is True


def test_audit_log_records_executions(sandbox):
    exec_task("read", "echo audit-test", str(sandbox))
    exec_task("read", "rm /etc/passwd", str(sandbox))  # denied
